...
class WpAcfVcBridge {
  ...
  private function __construct() {
    ...
    if ( is_admin() ) {
      // Since 1.4.16
      add_action( 'init', array( $this, 'integrate_vc_grid_params' ) );
    }
  }
  ...
  public function adminNotices() {
    if ( ! $this->is_vc_active ) {
      self::displayAdminNotice( 
        sprintf( '<a href="https://codecanyon.net/item/visual-composer-page-builder-for-wordpress/242431?ref=pavelreva" target="_blank">%s</a> %s', __( 'Visual Composer', 'wp-acf-vc-bridge' ), __( 'is required for the WP ACF-VC Bridge!', 'wp-acf-vc-bridge' ) ), 
        'error', 
        true
      );
    }
